...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <h4>Listado de Colores de Vehiculos</h4>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($vehiclecolors as $vehiclecolor)
                        <tr>
                            ...
                            <td>
                                <i class="fas fa-square" style="color: {{ $vehiclecolor->name }}; margin-right: 5px;"></i>
                                {{ $vehiclecolor->name }}
                            </td>
                            <td>{{ $vehiclecolor->description }}</td>

                            <td>
                                {{-- <a href="{{ route('admin.vehiclecolors.edit', $vehiclecolor->id) }}" class="btn btn-primary"><i
                                        class="fas fa-edit"></i></a> --}}
                                <a class="btn btn-primary btnEditar" data-id="{{ $vehiclecolor->id }}"><i
                                        class="fas fa-edit"></i></a>
                            </td>
                            <td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.btnEditar').click(function() {
                var id = $(this).data('id');
                $.ajax({
                    url: "{{ route('admin.vehiclecolors.edit', ['vehiclecolor' => '__id__']) }}"
                        .replace('__id__',
                            id),
                    type: "GET",
                    success: function(response) {
